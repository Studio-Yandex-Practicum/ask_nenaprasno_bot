...
WEEKLY_STAT_WEEK_DAYS = get_const_datetime_tuple("WEEKLY_STAT_WEEK_DAYS")  # дни недели для еженедельной статистики 0-6, где 0 - воскресенье
...
MONTHLY_RECEIPT_REMINDER_TIME = get_const_datetime("MONTHLY_RECEIPT_REMINDER_TIME")  # время для ежемесячного напоминания о чеке
MONTHLY_RECEIPT_REMINDER_DAY = get_const_int("MONTHLY_RECEIPT_REMINDER_DAY")  # день для даты ежемесячного напоминания о чеке
...
IS_FAKE_API = get_const_string("IS_FAKE_API")  # флаг, определяющий какой АПИ клиент используется - боевой или "заглушка"
